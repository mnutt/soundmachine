...
function PagePlayer(oConfigOverride) {
  ...
  this.setAlbumCover = function(o) {
    var searchString = jQuery(o).text().replace(/\s/ig, "%2B");
    var query = 'http://query.yahooapis.com/v1/public/yql?q=select%20*%20from%20html%20where%20url%3D%22http%3A%2F%2Fwww.store.limewire.com%2Fstore%2Fapp%2Fpages%2Fsearch%2FSearchResultsAll%2FsearchQuery%2F'+searchString+'%2F%22%20and%0A%20%20%20%20%20%20xpath%3D\'%2F%2Fdiv%5B%40id%3D%22bubble_albums%22%5D%2F%2Fdiv%5B%40class%3D%22album_small%22%5D%2F%2Fimg\'%0A%20%20%20%20&format=json&callback=?'
    jQuery.getJSON(query, function(result) {
      if(result && result.query && result.query.results && result.query.results.img) {
	var img = result.query.results.img[0];
	var src = img.src.replace(/cover50/, "cover160");
	jQuery("#cover-art-container").html("<img src='"+src+"'/>");
      }
    });
  }
